...
const PAL = [
  ...
  '#ff3040',
  '#ff3040',
  '#ff3040', // red (dominant like real tracker)
  '#ff6080',
  '#ff6080',
  '#ff6080', // pink
  '#ffaa33', // orange
...
function makeSats(n: number): Sat[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (b < 0.75) {
      r = 1.05 + Math.random() * 0.3;
      inc = ((25 + Math.random() * 75) * Math.PI) / 180;
    } else if (b < 0.88) {
      r = 1.4 + Math.random() * 0.25;
      inc = ((25 + Math.random() * 65) * Math.PI) / 180;
    } else if (b < 0.96) {
      r = 1.72 + Math.random() * 0.06;
      inc = (Math.random() * 10 * Math.PI) / 180;
    } else {
      r = 1.15 + Math.random() * 0.65;
    ...
    out.push({
      ...
      spd: (0.1 + Math.random() * 0.1) / Math.pow(r, 1.5),
    ...
    const draw = () => {
      ...
      const satPos = (s: Sat, time: number) => {
        ...
        let px = s.r * Math.cos(angle),
          py = 0,
          pz = s.r * Math.sin(angle);
        let r1 = rotX(px, py, pz, s.inc);
        px = r1.x;
        py = r1.y;
        pz = r1.z;
        let r2 = rotY(px, py, pz, s.raan);
        px = r2.x;
        py = r2.y;
        pz = r2.z;
        let r3 = rotX(px, py, pz, TILT);
        px = r3.x;
        py = r3.y;
        pz = r3.z;
