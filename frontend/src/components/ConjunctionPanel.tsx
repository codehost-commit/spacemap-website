...
const HELP: Record<string, string> = {
  ...
  "Rel. speed": "Magnitude of their relative velocity right now. Head-on can exceed 14 km/s; co-orbital cluster-mates are near 0.",
  TCA: "Time of Closest Approach — the exact instant during the next 24 h when they get nearest each other.",
  "Miss @ TCA": "3-D distance at TCA. Real collisions require this to be smaller than the two spacecraft's combined radius (a few metres).",
  ...
  Severity: "UI meter only (0–100). Blends miss distance and Pc so you can eyeball dangerous conjunctions. NOT a formal risk metric — Pc is.",
};
      ...
      {showHelp && (
        <div className="mb-3 space-y-2 rounded border border-space-border bg-space-bg/60 p-2 font-mono text-[10px] leading-snug text-space-text">
          <p className="text-space-dim">
            A <b>conjunction</b> is a close approach between two orbiting objects — not
            necessarily a collision. Values below describe how close and how risky the
            encounter is:
          </p>
      ...
      {compare == null ? (
        ...
        <div className="mb-2 font-mono text-xs">
          <div className="mb-1 flex items-center justify-between">
            ...
            <span className="truncate text-space-text">
              {names.get(compare) ?? `#${compare}`}
            </span>
          </div>
          ...
          {conjunction && (
            <div className="grid grid-cols-2 gap-x-3 gap-y-1">
              ...
              <Cell label="Rel. speed" value={`${conjunction.currentRelSpeedKmS.toFixed(3)} km/s`} />
              <Cell label="TCA" value={new Date(conjunction.tcaMs).toISOString().slice(0, 19)} wide />
              ...
              <Cell
                label="Severity"
                value={<SeverityBar value={conjunction.severity} />}
              />
            </div>
          )}
        </div>
            ...
            {neighbors.map((n) => (
              <li key={n.noradId}>
                ...
                >
                  ...
                  <span className="ml-2 shrink-0 text-space-dim">
...
function Cell({
  ...
  return (
    <div className={wide ? "col-span-2 flex justify-between" : "flex flex-col"}>
      <span
        className="text-[9px] uppercase tracking-widest text-space-dim"
        title={tip}
      >
        {label}
...
function SeverityBar({ value }: { value: number }) {
  ...
  const color =
    clamped > 66 ? "bg-space-bad" : clamped > 33 ? "bg-space-warn" : "bg-emerald-400";
  return (
    <div className="flex items-center gap-2">
