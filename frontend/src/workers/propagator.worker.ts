...
type InMsg = LoadMsg | PropagateMsg | PingMsg | ConjunctionMsg;
...
function propagate(timeMs: number): void {
  ...
  for (let i = 0; i < cap; i++) {
    ...
    orbitClass[n] =
      ORBIT_CLASS_INDEX[classifyOrbit(alt, (rec.satrec.inclo * 180) / Math.PI, rec.satrec.ecco)];
    n++;
  }
...
function separationKm(
  ...
  if (
    !pa ||
    !pb ||
    typeof pa.position === 'boolean' ||
    typeof pa.velocity === 'boolean' ||
    typeof pb.position === 'boolean' ||
    typeof pb.velocity === 'boolean'
  )
    return null;
...
function conjunction(msg: ConjunctionMsg): void {
  ...
  const hbrKm = 0.02;
  const pc =
    Math.exp(-(missKm * missKm) / (2 * sigmaKm * sigmaKm)) *
